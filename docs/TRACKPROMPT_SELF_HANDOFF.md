...
The current artistic revision is the real-analysis, bounded Signal → Awakening → Departure → First Gate proof under `test-output/cinematic-v2-andromeda-revision-20260722-134911/`. It contains six 640×360 stills, six 320×180 phone stills, and a verified six-second H.264/AAC authored-motion review edit produced by the canonical high-level MCP preview entrypoint. A separate MCP render receipt hash-binds the R11 scene, deterministic ShotPlan, six source excerpts, ordered rendered frames, representative stills, encoder contract, and final MP4; the proof verifier independently recompiles the cue/config/template-bound plan before sealing manifests. Deterministic dead-moon/beacon, opening-chamber, monumental-corridor, and split-monolith Gate environments now establish distinct shape, depth, action, and color identities. All four revision-6 Director reviews pass mobile and story-differentiation gates with specific R11 findings, while `artistApproved` remains false. V1 outputs and authorization evidence were not changed; V2 calibration, production authorization, cloud provisioning, and full-track rendering were not started. See `docs/cinematic-visualizer-v2.md` for exact evidence, verification, limitations, and review commands.
