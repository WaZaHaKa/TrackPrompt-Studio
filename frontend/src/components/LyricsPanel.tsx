...
import type { LyricsAnalysisSummary, LyricsSegment, PrivateLyricsTranscript } from '../types'
...
function SegmentRow({ segment, busy, onUpdate, onSeek }: {
...
export function LyricsPanel({ jobId, summary }: LyricsPanelProps) {
  const [transcript, setTranscript] = useState<PrivateLyricsTranscript>()
  const [loading, setLoading] = useState(Boolean(summary?.transcriptAvailable))
  ...
  useEffect(() => {
    if (!summary?.transcriptAvailable) return
    let active = true
    ...
    return () => { active = false }
  }, [jobId, summary?.transcriptAvailable])
  ...
  return (
    <section className="lyrics-panel" aria-labelledby="lyrics-panel-heading">
      ...
      {summary ? <dl className="diagnostic-grid"><div><dt>Status</dt><dd>{summary.status}</dd></div><div><dt>Language</dt><dd>{summary.language ?? 'unknown'} ({summary.languageConfidence})</dd></div><div><dt>Model/device</dt><dd>{summary.modelId ?? 'unavailable'} Â· {summary.selectedDevice}</dd></div><div><dt>Vocal density</dt><dd>{summary.vocalWordDensity ?? 'unknown'}</dd></div></dl> : null}
      {error ? <InlineNotice tone="error">{error}</InlineNotice> : null}
      {loading ? <p>Loading the private transcriptâ€¦</p> : null}
      {transcript ? (
        <>
          <audio ref={audioRef} controls preload="metadata" src={audioUrl(jobId)} aria-label="Private source track playback" />
          <div className="lyrics-segment-list">{transcript.segments.map((segment) => <SegmentRow key={segment.id} segment={segment} busy={busy} onUpdate={updateSegment} onSeek={seek} />)}</div>
      ...
      <label className="theme-editor">Approved abstract themes<textarea value={themes} onChange={(event) => setThemes(event.target.value)} placeholder="persistence and late-night movement, introspection and uncertainty" /></label>
      <Button disabled={busy} onClick={() => void saveThemes()}>Save abstract themes</Button>
