...
function Invoke-CapturedChecked {
    ...
    # Windows PowerShell 5.1 converts redirected native stderr into PowerShell
    # ErrorRecord objects. With the script-wide ErrorActionPreference set to
    # Stop, normal Docker Compose progress such as "Container ... Creating" can
    # incorrectly terminate the script even when Docker is succeeding.
    #
    # Temporarily use Continue while the native process runs, then decide
